...
public class HammerConfig {
	public static final Toml properties = new Toml("HammerMod Configs.toml \n[!] Be careful with IDs. Changes can affect your existing worlds.");
	...
	static void Setup() {
		...
		List<Field> itemFields = Arrays.stream(HammerItems.class.getDeclaredFields()).filter((F)-> Item.class.isAssignableFrom(F.getType())).collect(Collectors.toList());
